const MovieCard = ({ movie }) => {
  return (
    <div className="p-4 bg-dark-100 rounded-xl flex flex-col gap-2">
      <img className="rounded-xl" src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt="image" />
      <span className="text-light-100 text-sm">{movie.title}</span>
      <div className="flex items-center gap-2">
        ...
        <span className="text-light-100 text-sm">{movie.vote_average.toFixed(2)}</span>
        <span className="text-light-100">•</span>
        <span className="text-gray-100/60 text-sm">{movie.original_language.toUpperCase()}</span>
        <span className="text-light-100">•</span>
        <span className="text-gray-100/60 text-sm">{movie.release_date.slice(0,4)}</span>
